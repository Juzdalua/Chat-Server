...
{
	if (SQLAllocHandle(SQL_HANDLE_DBC, henv, &_connection) != SQL_SUCCESS) return false;
	...
	if (SQLAllocHandle(SQL_HANDLE_STMT, _connection, &_statement) != SQL_SUCCESS) return false;
...
bool DBConnection::BindParam(SQLUSMALLINT parameterIndex, SQLSMALLINT cType, SQLSMALLINT sqlType, SQLLEN len, SQLPOINTER ptr, SQLLEN* index)
{
	SQLRETURN ret = SQLBindParameter(_statement, parameterIndex, SQL_PARAM_INPUT, cType, sqlType, len, 0, ptr, 0, index);
